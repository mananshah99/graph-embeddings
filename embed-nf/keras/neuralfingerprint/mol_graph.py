...
def graph_from_edgelist(edgelist):
    ...
    for edge in edges:
        node_1 = graph.new_node('atom', features=np.random.uniform(low=0, high=1, size=(62,)), rdkit_ix=int(edge[0]))
        node_2 = graph.new_node('atom', features=np.random.uniform(low=0, high=1, size=(62,)), rdkit_ix=int(edge[1]))
        node_1.add_neighbors((node_2,))
        node_2.add_neighbors((node_1,))

        edge   = graph.new_node('bond', features=np.random.uniform(low=0, high=1, size=(6, )))
