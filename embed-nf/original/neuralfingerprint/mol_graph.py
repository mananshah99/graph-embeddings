...
from features import atom_features, bond_features, num_atom_features, num_bond_features
import re
...
def graph_from_edgelist_tuple(edgelist_tuple):
    graph_list = [graph_from_edgelist(s) for s in edgelist_tuple]
    big_graph = MolGraph()
    for subgraph in graph_list:
        big_graph.add_subgraph(subgraph)
...
def graph_from_edgelist(edgelist):
    ...
    for edge in edges:
        node_1 = graph.new_node('atom', features=np.random.uniform(low=0, high=1, size=(num_atom_features(),)), rdkit_ix=int(edge[0]))
        node_2 = graph.new_node('atom', features=np.random.uniform(low=0, high=1, size=(num_atom_features(),)), rdkit_ix=int(edge[1]))
        node_1.add_neighbors((node_2,))

        edge   = graph.new_node('bond', features=np.random.uniform(low=0, high=1, size=(num_bond_features(), )))
